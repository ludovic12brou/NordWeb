...
export default function Portfolio() {
  ...
  return (
    <section
      id="portfolio"
      className="py-24 bg-[#0E2A4F] relative overflow-hidden"
    >
      ...
      <div className="container relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div
              key={index}
              className="group bg-slate-900/80 border border-white/10 rounded-lg overflow-hidden transition-colors duration-200 hover:border-white/20"
            >
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 bg-orange-500/10 text-orange-500 text-xs font-semibold rounded-md border border-orange-500/20"
                    >
                      {tag}
                    </span>
                  ))}
